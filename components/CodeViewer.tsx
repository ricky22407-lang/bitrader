...
const CodeViewer: React.FC<CodeViewerProps> = ({ content, isGenerating }) => {
  ...
  const handleDownload = () => {
    if (!content) return;
    const blob = new Blob([content.code], { type: 'text/x-python;charset=utf-8' });
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-slate-800 border-b border-slate-700">
        ...
        <div className="flex items-center gap-2">
           <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-slate-300 bg-slate-700 hover:bg-slate-600 hover:text-white rounded-md transition-colors"
            title="下載 bot.py"
           >
